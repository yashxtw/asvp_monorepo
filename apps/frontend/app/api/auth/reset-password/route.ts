...
export async function POST(req: NextRequest) {
    ...
    const { backendResponse, json, errorResponse } = await proxyBackendAuthRequest(req, "/auth/reset-password", {
        method: "POST",
        body,
    });

    if (errorResponse || !backendResponse) {
        return errorResponse ?? NextResponse.json({ error: "Failed to reset password" }, { status: 500 });
    }
